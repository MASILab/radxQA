function [centre_coord,contour_xy,I_bin]=MakeEllipseROI_EPI(I,phan_centre,phan_bndry,FOV_bndry,wl_pxl,pxl_sz,choice)

%1.check if there is enough space to create ROI between phantom and FOV
empty_space=abs(phan_bndry-double(FOV_bndry));
ROI_phan_distance=1;%HW:default ROI to phantom distance
if empty_space>wl_pxl(1,1)+ROI_phan_distance+1%v2
    ...
    l_pxl=4*1000/(pi*w_pxl*pxl_sz(1,1))/pxl_sz(2,1);%make sure 10cm2 area%v2
...
x(isnan(x))=-1; y(isnan(y))=-1;
x(isinf(x))=-1; y(isinf(y))=-1;
I_bin=roipoly(I,x,y);

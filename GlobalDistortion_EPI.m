function  [A,B,C,D,Distortion,slice] = GlobalDistortion_EPI(img,reso)

% get slice with most "mask"
BZ = mean(img(:,:,:,:),4);
BZ = mat2gray(BZ);
% level = graythresh(BZ(:));
% mask_glass = im2bw(BZ(:),0.8*level);
% mask = reshape(mask_glass,size(BZ));

% grab slice with most ones
one_sum = squeeze(sum(sum(BZ,1),2));
slice = find(one_sum==max(one_sum));

% in this case, just select middle temporal time point

I = squeeze(img(:,:,slice,round(size(img,4)/2)));

...
for i in $(find /blazepool/radx/QA/NEWDATA/* -type d -mindepth 1 -maxdepth 1 -mmin +2); do
	# ** may have issues with spaces in file names ** 
	# for i in /GRAID/BlazeFake/NEWDATA/
	echo $i;
	# only if EPI_STABILITY or T1_axial or ACR_T1
	if [[ $i == *"EPI_STABILITY" ]] || [[ $i == *"EPI-STABILITY" ]] || [[ $i == *"T1_axial" ]] || [[ $i == *"T1-axial" ]] || [[ $i == *"ACR" ]]; then
		...
		then echo 'Already Done...'; 
			...
			/blazepool/radx/QA/radxQA/run_dcmdjpeg.sh $i
			# run mcverter on each
			...
			mv $i/decompressed_dicoms/OUT/* /blazepool/radx/QA/NIFTIS

function [Shewart,message] = ShewartEval_v2(Val_sorted)
% v2 uses last 29 measures

% 1: mean+/- 2D of previous measures
if abs(Val_sorted(end) - mean(Val_sorted(end-29:end-1))) > 2*std(Val_sorted(end-20:end-1))
...
if Shewart == 1
    % 2: exceeds 3D
    if abs(Val_sorted(end) - mean(Val_sorted(end-29:end-1))) > 3*std(Val_sorted(end-20:end-1))
        Shewart = 2; message = 'Out of 3*SD limits: May need evaluation';
    end
    % 3: two consecutive measures that exeed mean + 2STD
    if sum(abs(Val_sorted(end-1:end) - mean(Val_sorted(end-29:end-2))) > 2*std(Val_sorted(end-29:end-2)))==2
    ...
    if abs(Val_sorted(end)-Val_sorted(end-1))>4*std(Val_sorted(end-29:end-2))
        Shewart = 4; message = 'Big Change in system: Requires Evaluation';
    end
    % 5: 4 consectuive measures exeed same limit of 1SD or -1 SD
    if sum(abs(Val_sorted(end-3:end) - mean(Val_sorted(end-29:end-4))) > 1*std(Val_sorted(end-29:end-4)))==4
